...
ipcMain.on('get-entries', async (event, arg) => {
  try {
    let entradas = await Entrada.findAll();

    event.reply('get-entries', entradas);
  } catch(err) {
    console.log(err);
  }
...
ipcMain.on('post-entries', async (event, arg) => {
  try {
    for (const entry of arg) {
      await Entrada.create(entry);
    }

    let entradas = await Entrada.findAll();

    event.reply('post-entries', entradas);
  } catch(err) {
    console.log(err);
  }
});
